    ...
    if (mode === "search") {
      ...
      return (
        <>
          ...
          <table className="table-many-column mt-5">
            <thead>
              <tr>
                <th className="font text-center" style={{ minWidth: "30px" }}>#</th>
                <th className="font" style={{ minWidth: "130px" }}>ชื่อรายการบำรุงรักษา</th>
                <th className="font" style={{ minWidth: "250px" }}>สถานะ</th>
            ...
            <tbody>
              {this.props.list_show_main.map(function (list_show, row) {
                ...
                <td className="edit-padding text-left" style={{ minWidth: "250px" }}>{list_show.no_part}</td>
          ...
          <table className="table-many-column mt-5">
            ...
            <tbody>
              {this.props.list_show_main.map(function (list_show, row) {
                ...
                <td className="edit-padding text-left" style={{ minWidth: "250px" }}>{list_show.no_part}</td>
                <td className="edit-padding text-center" style={{ minWidth: "80px" }}>
                  <select className="edit-select-table" onChange={(e) => current.props.onChangeStatusMainEachRow(e)}>
                    {current.props.list_unit.map(function (list_unit, index) {
                          if (list_show.status === list_unit.status) {
        ...
        <>
          ...
          <table className="table-many-column mt-5">
            ...
            {this.props.list_show_mode_add_main.map(function (list_show, row) {
                ...
                <td className="edit-padding text-left" style={{ minWidth: "250px" }}>{list_show.no_part}</td>
                <td className="edit-padding text-center" style={{ minWidth: "80px" }}>
                  <select className="edit-select-table" onChange={(e) => current.props.onChangeStatusMainEachRowAdd(e)}>
                    {current.props.list_unit.map(function (list_unit, index) {
                          if (list_show.status === list_unit.status) {
                            return <option defaultValue={list_unit.id} key={index} selected> {list_unit.status} </option>
                          }
...
const mapDispatchToProps = (dispatch) => ({
  ...
  onChangeStatusMainEachRow: (e) => dispatch(onChangeStatusMainEachRow(e)),
  onChangeNoteMainEachRow: (e) => dispatch(onChangeNoteMainEachRow(e)),
  onChangeStatusMainEachRowAdd: (e) => dispatch(onChangeStatusMainEachRowAdd(e)),
...
export const onChangeStatusMainEachRow = (e) => {
  return {
    type: "ON CHANGE STATUS MAIN EACH ROW",
    value: e.target.value,
    rowIndex: e.target.parentNode.parentNode.id
  }
}
...
export const onChangeStatusMainEachRowAdd = (e) => {
  return {
    type: "ON CHANGE STATUS MAIN EACH ROW MODE ADD",
    value: e.target.value,
    rowIndex: e.target.parentNode.parentNode.id
  }
}
