...
import { getEmployeeIDFromUserID, fetchStepApprovalDocumentData, DOCUMENT_TYPE_ID, DOCUMENT_TYPE_NOTGROUP_ID, validateEmployeeIDField, validateWarehouseIDField, validateInternalDocumentIDFieldHelper } from '../../helper';
...
const TopContent = (props) => {
    ...
    const validateInternalDocumentIDField = (...args) => validateInternalDocumentIDFieldHelper(DOCUMENT_TYPE_ID.MAINTENANT_ITEM, toolbar, footer, fact, values, setValues, setFieldValue, validateField, ...args);
            ...
            line_items.push(
                {
                    ...
                    internal_item_id: data.line_items[0].internal_item_id,
                    description: data.line_items[0].description,
                    quantity: data.line_items[0].quantity,   //ของเสีย
                    quantity_fix: '',
                    quantity_salvage: "",
                    uom_group_id: data.line_items[0].uom_group_id,
                    unit: data.line_items[0].unit,
                    list_uoms: data.line_items[0].list_uoms
                }
            );
        }
        return line_items;
    ...
    const validateInternalDocumentGoodFixID = refer_to_document_internal_document_id => new Promise(resolve => {
    ...
    return (
        <div id="blackground-white">
            ...
            <PopupModalDocument
                documentTypeGroupID={DOCUMENT_TYPE_NOTGROUP_ID.MAINTENANT_ITEM}
