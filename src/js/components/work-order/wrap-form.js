...
class WrapForm extends React.Component {
    ...
    packForm = (document_id, document_show) => {
        const current = this;
        // const line_items = [];
        // var line_number = 1
    ...
    handleSubmit = e => {
        ...
        if (this.props.actionMode === "add") {
            console.log(this.packForm(this.props.document_id, this.props.document_show_mode_add));
            const formData = new FormData();
            
            return (
                axios.put(`http://${API_URL_DATABASE}:${API_PORT_DATABASE}/document/${this.props.document_id}/201`, this.packForm(this.props.document_id, this.props.document_show_mode_add), { headers: { "x-access-token": localStorage.getItem('token_auth') } })
                    .then(res => {
                        console.log(res);
        ...
        if (this.props.actionMode === "edit") {
            console.log(this.packForm(this.props.document_show.document_id, this.props.document_show));
            console.log(this.props.document_show)
            return (
                axios.put(`http://${API_URL_DATABASE}:${API_PORT_DATABASE}/document/${this.props.document_show.document_id}/201`, this.packForm(this.props.document_show.document_id, this.props.document_show), { headers: { "x-access-token": localStorage.getItem('token_auth') } })
                    .then(res => {
                        console.log(res);
                        this.props.onClearStateModeAdd()
                    }).catch(function (err) {
