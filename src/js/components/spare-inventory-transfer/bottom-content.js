    ...
    fetchGoodsOnhandData(getNumberFromEscapedString(values.src_warehouse_id), values.line_items[index].item_id)
      .then((at_source) => {
        ...
        if (at_source) {
          ...
          setFieldValue(`line_items[${index}].per_unit_price`, at_source.pricing.fifo.length > 0 ? weightedAverage(getLotFromQty(rawLotFromQty(at_source.pricing.fifo, at_source.current_unit_count), values.line_items[index].quantity)) : "0.00", false);
        }
        else {
          console.log(" NOT FOUND AT SOURCES FOR CALCULATE FIFO")
